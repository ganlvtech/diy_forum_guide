 ...
 * 自定义论坛导读
 *
 * upload/source/plugin/diy_forum_guide/module/forum/forum_guide.php
 *
 * 修改自 upload/source/module/forum/forum_guide.php
 *
 * @author Ganlv
 */
...
// <<<<<<<<<< added
$lang['guide_' . $view] = strip_tags($_G['cache']['plugin']['diy_forum_guide']['views'][$view]['title']);
$lang['guide_' . $view . '_description'] = $_G['cache']['plugin']['diy_forum_guide']['views'][$view]['data']['description'];
// >>>>>>>>>>
...
if ($_GET['rss'] == 1) {
    ...
    if (empty($info) || (TIMESTAMP - $info[0]['lastupdate'] > $ttl * 60)) {
        diy_forum_guide_update_guide_rsscache($view, $perpage);
    }
...
if ($view != 'index') {
    ...
    if ($view == 'my') {
        ...
        if ($searchkey = stripsearchkey($_GET['searchkey'])) {
            $searchkey = dhtmlspezcialchars($searchkey);
        }
        ...
        $data['my'] = diy_forum_guide_get_my_threads($viewtype, $_GET['fid'], $filter, $searchkey, $start, $perpage, $theurl);
        $tids = $data['my']['tids'];
        $posts = $data['my']['posts'];
    } else {
        $data[$view] = diy_forum_guide_get_guide_list($view, $start, $perpage);
    }
    ...
    $data['hot'] = diy_forum_guide_get_guide_list('hot', 0, 30);
    $data['digest'] = diy_forum_guide_get_guide_list('digest', 0, 30);
    $data['new'] = diy_forum_guide_get_guide_list('new', 0, 30);
    $data['newthread'] = diy_forum_guide_get_guide_list('newthread', 0, 30);
}
...
function diy_forum_guide_get_guide_list($view, $start = 0, $num = 50, $again = 0)
{
    global $_G;
    ...
    // <<<<<<<<<< added
    $cachetimelimit = plugin_diy_forum_guide_forum::get_view_cache_time_limit($cachetimelimit);
    // >>>>>>>>>>
    ...
    // ==========
    $query = plugin_diy_forum_guide_forum::forum_thread_fetch_all_for_guide($view, $limittid, $tids, $_G['setting']['heatthread']['guidelimit'], $dateline);
    // >>>>>>>>>>
    $n = 0;
    foreach ($query as $thread) {
        ...
        $thread = diy_forum_guide_guide_procthread($thread);
    ...
    if ($limittid > $maxnum && !$again && count($list) < 50) {
        return diy_forum_guide_get_guide_list($view, $start, $num, 1);
    }
    $forumnames = [];
    if ($fids) {
...
function diy_forum_guide_get_my_threads($viewtype, $fid = 0, $filter = '', $searchkey = '', $start = 0, $perpage = 20, $theurl = '')
{
    global $_G;
    ...
    if ($viewtype == 'thread') {
        ...
        foreach (C::t('forum_thread')->fetch_all_by_authorid_displayorder($authorid, $displayorder, $dglue, $closed, $searchkey, $start, $perpage, null, $fid) as $tid => $value) {
            ...
            $list[$value['tid']] = diy_forum_guide_diy_forum_guide_guide_procthread($value);
        }
        ...
        foreach ($postcommentarr as $value) {
            ...
            $list[] = diy_forum_guide_guide_procthread($value);
        }
        ...
        if (!empty($tids)) {
            ...
            foreach ($threads as $tid => $thread) {
                ...
                $threads[$tid] = diy_forum_guide_guide_procthread($thread);
            }
            if (!empty($gids)) {
...
function diy_forum_guide_guide_procthread($thread)
{
    global $_G;
...
function diy_forum_guide_update_guide_rsscache($type, $perpage)
{
    global $_G;
    ...
    require_once libfile('function/post');
    $data = diy_forum_guide_get_guide_list($type, 0, $perpage);
